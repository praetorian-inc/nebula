...
func (l *Neo4jImporterLink) createValidatedEscalationEdges() bool {
	message.Info("Creating validated CAN_ESCALATE edges - 22 production-ready attack vectors...")
	...
	validatedQueries := []struct {
		...
		// DIRECTORY ROLES (11 vectors) - Use HAS_PERMISSION.roleName/templateId
...
func (l *Neo4jImporterLink) getValidatedGlobalAdminQuery() string {
	...
	MERGE (user)-[r:CAN_ESCALATE {method: "GlobalAdministrator"}]->(escalate_target)
	ON CREATE SET r.method = "GlobalAdministrator",
...
func (l *Neo4jImporterLink) getValidatedPrivilegedRoleAdminQuery() string {
	...
	WITH DISTINCT user, escalate_target, perm
	MERGE (user)-[r:CAN_ESCALATE {method: "PrivilegedRoleAdmin"}]->(escalate_target)
	ON CREATE SET r.method = "PrivilegedRoleAdmin",
...
// getValidatedPrivilegedAuthAdminQuery - Privileged Authentication Administrator can reset ANY user's passwords
func (l *Neo4jImporterLink) getValidatedPrivilegedAuthAdminQuery() string {
	...
	WITH DISTINCT user, escalate_target, perm
	MERGE (user)-[r:CAN_ESCALATE {method: "PasswordResetViaPrivilegedAuthAdmin"}]->(escalate_target)
	ON CREATE SET r.method = "PasswordResetViaPrivilegedAuthAdmin",
...
func (l *Neo4jImporterLink) getValidatedApplicationAdminQuery() string {
	...
	MERGE (user)-[r:CAN_ESCALATE {method: "ApplicationAdmin"}]->(app)
	ON CREATE SET r.method = "ApplicationAdmin",
...
func (l *Neo4jImporterLink) getValidatedCloudApplicationAdminQuery() string {
	...
	WHERE toLower(app.resourceType) IN ["microsoft.directoryservices/applications", "microsoft.directoryservices/serviceprincipals"]
	WITH DISTINCT user, app, perm
	MERGE (user)-[r:CAN_ESCALATE {method: "CloudApplicationAdmin"}]->(app)
	ON CREATE SET r.method = "CloudApplicationAdmin",
...
func (l *Neo4jImporterLink) getValidatedGroupsAdminQuery() string {
	...
	MERGE (user)-[r:CAN_ESCALATE {method: "GroupsAdministrator"}]->(group)
	ON CREATE SET r.method = "GroupsAdministrator",
...
func (l *Neo4jImporterLink) getValidatedUserAdminQuery() string {
	...
	WHERE escalate_target <> user
	  ...
	  AND (
	    ...
	    OR
	    // OR users with ONLY roles from the allowed whitelist
	    (
	...
	WITH DISTINCT user, escalate_target, perm
	MERGE (user)-[r:CAN_ESCALATE {method: "PasswordResetViaUserAdmin"}]->(escalate_target)
	ON CREATE SET r.method = "PasswordResetViaUserAdmin",
	    r.condition = "Can reset passwords and modify properties of users with no admin roles or specific low-privilege admin roles (Directory Readers, Groups Administrator, Guest Inviter, Helpdesk Administrator, Message Center Reader, Password Administrator, Reports Reader, User Administrator, User Experience Success Manager, Usage Summary Reports Reader)",
	    r.category = "DirectoryRole",
	    r.sourcePermission = perm.source,
	    r.viaGroup = perm.viaGroupName,
	    r.grantedByGroups = perm.grantedByGroups,
...
// getValidatedAuthenticationAdminQuery - Authentication Administrator for non-admin users and specific low-privilege admin roles
func (l *Neo4jImporterLink) getValidatedAuthenticationAdminQuery() string {
	  ...
	  AND (
	    ...
	    (
	      ...
	      AND NOT EXISTS {
	        (escalate_target)-[admin_perm:HAS_PERMISSION]->(:Resource)
	        WHERE NOT admin_perm.templateId IN [
	          "c4e39bd9-1100-46d3-8c65-fb160da0071f",  // Authentication Administrator (self)
	          "88d8e3e3-8f55-4a1e-953a-9b9898b8876b",  // Directory Readers
	          "95e79109-95c0-4d8e-aee3-d01accf2d47b",  // Guest Inviter
	          "790c1fb9-7f7d-4f88-86a1-ef1f95c05c1b",  // Message Center Reader
	          "966707d0-3269-4727-9be2-8c3a10f19b9d",  // Password Administrator
	...
	ON CREATE SET r.method = "PasswordResetViaAuthAdmin",
	    r.condition = "Can reset passwords and authentication methods for users with no admin roles or specific low-privilege admin roles (Authentication Administrator, Directory Readers, Guest Inviter, Message Center Reader, Password Administrator, Reports Reader, User Experience Success Manager, Usage Summary Reports Reader)",
	    r.category = "DirectoryRole",
	    r.sourcePermission = perm.source,
	    r.viaGroup = perm.viaGroupName,
	    r.grantedByGroups = perm.grantedByGroups,
...
func (l *Neo4jImporterLink) getValidatedPasswordAdminQuery() string {
	...
	WHERE escalate_target <> user
	  ...
	  AND (
	    ...
	    NOT EXISTS { (escalate_target)-[:HAS_PERMISSION]->(:Resource) }
	    OR
	    // OR users with ONLY allowed roles from whitelist
	    (
	      EXISTS { (escalate_target)-[:HAS_PERMISSION]->(:Resource) }
	      AND NOT EXISTS {
	        (escalate_target)-[admin_perm:HAS_PERMISSION]->(:Resource)
	...
	ON CREATE SET r.method = "PasswordResetViaPasswordAdmin",
	    r.condition = "Can reset passwords for users with no admin roles or specific low-privilege roles (Directory Readers, Guest Inviter, Password Administrator)",
	    r.category = "DirectoryRole",
	    r.sourcePermission = perm.source,
	    r.viaGroup = perm.viaGroupName,
	    r.grantedByGroups = perm.grantedByGroups,
...
func (l *Neo4jImporterLink) getValidatedHelpdeskAdminQuery() string {
	...
	ON CREATE SET r.method = "PasswordResetViaHelpdeskAdmin",
	    r.condition = "Can reset passwords for users with no admin roles or specific low-privilege roles (Directory Readers, Guest Inviter, Helpdesk Administrator, Message Center Reader, Password Administrator, Reports Reader, User Experience Success Manager, Usage Summary Reports Reader)",
	    r.category = "DirectoryRole",
	    r.sourcePermission = perm.source,
	    r.viaGroup = perm.viaGroupName,
	    r.grantedByGroups = perm.grantedByGroups,
...
func (l *Neo4jImporterLink) getValidatedGraphRoleManagementQuery() string {
	...
	MERGE (sp)-[r:CAN_ESCALATE {method: "GraphRoleManagement"}]->(escalate_target)
	ON CREATE SET r.method = "GraphRoleManagement",
...
func (l *Neo4jImporterLink) getValidatedGraphDirectoryReadWriteQuery() string {
	...
	WITH DISTINCT sp, escalate_target, perm
	MERGE (sp)-[r:CAN_ESCALATE {method: "Directory.ReadWrite.All"}]->(escalate_target)
	ON CREATE SET r.method = "Directory.ReadWrite.All",
...
func (l *Neo4jImporterLink) getValidatedGraphApplicationReadWriteQuery() string {
	...
	WITH DISTINCT sp, escalate_target, perm
	MERGE (sp)-[r:CAN_ESCALATE {method: "GraphApplicationReadWrite"}]->(escalate_target)
	ON CREATE SET r.method = "GraphApplicationReadWrite",
...
func (l *Neo4jImporterLink) getValidatedGraphAppRoleAssignmentQuery() string {
	...
	WHERE perm.source = "Microsoft Graph"
	  ...
	  AND perm.consentType = "AllPrincipals"
	WITH sp, perm
	MERGE (sp)-[r:CAN_ESCALATE {method: "GraphAppRoleAssignment"}]->(sp)
	ON CREATE SET r.method = "GraphAppRoleAssignment",
...
func (l *Neo4jImporterLink) getValidatedGraphUserReadWriteQuery() string {
	...
	MERGE (sp)-[r:CAN_ESCALATE {method: "GraphUserReadWrite"}]->(user)
	ON CREATE SET r.method = "GraphUserReadWrite",
...
func (l *Neo4jImporterLink) getValidatedGraphGroupReadWriteQuery() string {
	...
	MERGE (sp)-[r:CAN_ESCALATE {method: "GraphGroupReadWrite"}]->(group)
	ON CREATE SET r.method = "GraphGroupReadWrite",
...
func (l *Neo4jImporterLink) getValidatedRBACOwnerQuery() string {
	...
	MERGE (principal)-[r:CAN_ESCALATE {method: "AzureOwner"}]->(escalate_target)
	ON CREATE SET r.method = "AzureOwner",
...
func (l *Neo4jImporterLink) getValidatedRBACUserAccessAdminQuery() string {
	...
	WHERE escalate_target <> principal
	  AND NOT toLower(escalate_target.resourceType) STARTS WITH "microsoft.directoryservices/"
	WITH DISTINCT principal, escalate_target, perm
	MERGE (principal)-[r:CAN_ESCALATE {method: "UserAccessAdmin"}]->(escalate_target)
	ON CREATE SET r.method = "UserAccessAdmin",
...
func (l *Neo4jImporterLink) getValidatedSPOwnerAddSecretQuery() string {
	...
	ON CREATE SET r.method = "ServicePrincipalAddSecret",
...
func (l *Neo4jImporterLink) getValidatedAppOwnerAddSecretQuery() string {
	...
	ON CREATE SET r.method = "ApplicationAddSecret",
...
func (l *Neo4jImporterLink) getValidatedApplicationToServicePrincipalQuery() string {
	...
	ON CREATE SET r.method = "ApplicationToServicePrincipal",
...
func (l *Neo4jImporterLink) getValidatedManagedIdentityToServicePrincipalQuery() string {
	...
	ON CREATE SET r.method = "ManagedIdentityToServicePrincipal",
...
func (l *Neo4jImporterLink) getValidatedAzureResourceToManagedIdentityQuery() string {
	...
	MERGE (resource)-[r:CAN_ESCALATE {method: "ResourceAttachedIdentity"}]->(mi)
	ON CREATE SET r.method = "ResourceAttachedIdentity",
...
func (l *Neo4jImporterLink) getValidatedAzureResourceToUserAssignedMIQuery() string {
	...
	WITH DISTINCT resource, mi
	MERGE (resource)-[r:CAN_ESCALATE {method: "ResourceAttachedUserAssignedIdentity"}]->(mi)
	ON CREATE SET r.method = "ResourceAttachedUserAssignedIdentity",
